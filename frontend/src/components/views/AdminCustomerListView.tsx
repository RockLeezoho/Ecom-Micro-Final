  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 space-y-4 md:space-y-0">
        <div>
          <h1 className="text-3xl font-black text-gray-900">{t('manage_customers')}</h1>
          <p className="text-gray-500 font-medium">{t('manage_customers_desc')}</p>
        </div>
        <button
          onClick={onAdd}
        ...
        >
          <Plus size={18} className="mr-2" /> {t('create_customer')}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          <div className="col-span-full py-24 text-center text-gray-400 font-medium">{t('loading_customer_list')}</div>
            ...
            <div key={customer.id} className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-xl shadow-primary/5 transition-all">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex space-x-1">
                  ...
                  <button
                    onClick={() => {
                      if (window.confirm(t('confirm_delete_customer', { name: customer.name }))) {
              ...
              <div className="space-y-3 mb-8">
                ...
                <div className="flex items-center text-sm text-gray-500 font-medium">
                  <Phone size={16} className="mr-3 text-gray-300" /> {customer.phoneNumber || t('not_updated')}
                </div>
          ...
          <div className="col-span-full py-24 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-400">{t('no_customers_found')}</h3>
          </div>
        )}
      </div>
    </div>
