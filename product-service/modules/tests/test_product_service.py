...
def test_get_homepage_products_returns_structured_read_model():
    ...
    # Assert
    assert homepage.category.id == "c1"
    assert len(homepage.new_arrivals) == 1
    assert len(homepage.popular) == 1
    assert isinstance(homepage.recommended, list)
    assert isinstance(homepage.best_sellers, list)
    # ensure cache.set was called for ai and order keys
