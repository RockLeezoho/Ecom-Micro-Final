...
class JWTBearerAuthentication(BaseAuthentication):
    """
    Stateless JWT auth for microservices.

    - Verifies Bearer token signature (HS256 by default)
    - Does NOT require a local Django User table
    - Exposes request.user.id and request.user.role for permissions
    """
...
class InternalServiceAuthentication(BaseAuthentication):
    """Authenticate trusted internal service calls via X-Service-Token and X-User-Id."""
    ...
    def authenticate(self, request) -> Optional[Tuple[JWTUser, str]]:
        ...
        if not user_id:
            raise AuthenticationFailed("Missing X-User-Id for internal service call")

        return JWTUser(id=user_id, role="service"), token
